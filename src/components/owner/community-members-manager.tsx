...
type CommunityMembersManagerProps = {
  communityId: string;
  ...
  memberRemovalEndpointBase?: string;
};
...
export function CommunityMembersManager({
  ...
  inviteeLabel = "member",
  invitationEndpoint = `/api/communities/${communityId}/invitations`,
  memberRemovalEndpointBase = `/api/communities/${communityId}/members`,
  ...
  async function reviewRequest(requestId: string, nextStatus: string) {
    ...
    if (nextStatus === "ACCEPTED" && reviewed) {
      setMemberships((current) => [
        {
          memberId: reviewed.member.id,
          status: "ACTIVE",
          member: reviewed.member,
        },
        ...current,
      ]);
    }
  ...
  async function removeMember(memberIdToRemove: string) {
    const membership = memberships.find((item) => item.memberId === memberIdToRemove);
    const memberLabel =
      membership?.member.name ??
      membership?.member.email?.split("@")[0] ??
      `this ${inviteeLabel}`;

    if (!window.confirm(`Remove ${memberLabel} from this ${entityLabel}?`)) {
      return;
    }

    const response = await fetch(`${memberRemovalEndpointBase}/${memberIdToRemove}`, {
      method: "DELETE",
    });
    const payload = (await response.json().catch(() => null)) as
      | { error?: string }
    ...
    if (!response.ok) {
      setStatus(payload?.error ?? `Could not remove ${inviteeLabel}.`);
      return;
    }

    setMemberships((current) =>
      current.map((membership) =>
        membership.memberId === memberIdToRemove
          ? { ...membership, status: "REMOVED" }
          : membership,
      ),
    );
    setStatus(
      `${inviteeLabel.charAt(0).toUpperCase()}${inviteeLabel.slice(1)} removed from the ${entityLabel}.`,
    );
  }
  ...
  return (
    <section className="space-y-6">
      ...
      <div>
        ...
        <h1 className="mt-1 text-2xl font-bold">{communityTitle}</h1>
      </div>

      <section className="rounded-[24px] border border-[color:var(--line)] bg-white p-5 shadow-sm">
        <h2 className="text-lg font-semibold">Invite a {inviteeLabel}</h2>
              ...
              memberships.map((membership) => (
                ...
                >
                  <p className="font-semibold">
                    {membership.member.name ??
                      membership.member.email?.split("@")[0] ??
                      inviteeLabel.charAt(0).toUpperCase() + inviteeLabel.slice(1)}
                  </p>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    {membership.status === "ACTIVE" ? (
                      <button
                        type="button"
                        onClick={() => removeMember(membership.memberId)}
                ...
                >
                  <p className="font-semibold">
                    {invitation.member.name ??
                      invitation.member.email?.split("@")[0] ??
                      inviteeLabel.charAt(0).toUpperCase() + inviteeLabel.slice(1)}
                  </p>
                  <p className="text-sm text-[color:var(--muted)]">
